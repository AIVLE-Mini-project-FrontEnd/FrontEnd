...
function Navigation() {
  ...
  const NAV_LIST = [
    { title: '도서검색', items: ['통합검색'] },
    { title: '도서목록' },
    { title: '종류별', items: ['장르별', '태그별', '좋아요순'] },
    { title: '지원', items: ['공지사항', '자주 묻는 질문'] },
  ];

  const handleNavClick = (title) => {
    if (title === '도서목록') navigate('/books');
  };
    ...
    <div className="likes-book-wrap">
      ...
      <section className="likes-book-section">
        ...
        <div className="likes-book-slider">
          ...
          <button
            className="likes-book-btn left"
            onClick={() => movePrev(suggestBooks, suggestIndex, setSuggestIndex)}
          >
            ‹
          </button>
          ...
          <button
            className="likes-book-btn right"
            onClick={() => moveNext(suggestBooks, suggestIndex, setSuggestIndex)}
          >
            ›
          </button>
        </div>
  ...
  return (
    <section className="stats-section">
      ...
      <div className="stats-chart-wrap">
        {ChartCard('장르별 좋아요 수', genreStats, genreChartType, setGenreChartType)}
        {ChartCard('태그별 좋아요 수', tagStats, tagChartType, setTagChartType)}
      </div>
    </section>
  );
}
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: '8px', padding: '16px' }}>
        <button onClick={() => navigate('/books')}>도서 목록</button>
        <button onClick={() => navigate('/books/register')}>도서 등록</button>
        <button onClick={() => navigate('/books/deleted')}>삭제 도서</button>
      </div>
      <BookSection />
      <StatisticsSection />
    </>
